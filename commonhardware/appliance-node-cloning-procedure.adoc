...
summary: 克隆过程可能需要几天时间才能在源节点（正在更换的设备）和目标（新）设备之间传输数据。 
---
= 克隆 StorageGRID 设备节点
:allow-uri-read: 
:icons: font
...
+
如果存在阻止克隆的问题，则未启用 *开始克隆*，并且必须解决的问题将列为 *连接状态*。这些问题列在源节点和目标设备的 StorageGRID Appliance Installer 主页上。一次只显示一个问题，状态会随着条件的变化而自动更新。解决所有克隆问题以启用 *开始克隆*。
...
NOTE: 如果在克隆过程结束时遇到 IP 地址冲突，请确保没有为源节点上具有静态 IP 且在目标节点上启用了 DHCP 的端口的任何 MAC 地址分配永久 DHCP 租约。删除或更新 DHCP 服务器上的永久 DHCP 租约以解决此冲突。
