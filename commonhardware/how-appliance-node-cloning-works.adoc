...
* 源设备和目标设备不兼容
* 源设备和目标设备处于不同的维护模式状态
* 源设备和目标设备上的 StorageGRID Appliance Installer 版本不匹配
* 对于存储节点，目标设备的存储不兼容或容量不足
* 目标设备未就绪
...
** 目标存储设备中的卷数必须大于或等于源节点中的卷数。SGF6112 有 12 个卷，SGF6212 有 13 个卷。所有其他存储设备都有 16 个卷。
** 一般存储设备克隆注意事项的例外情况包括：
+
...
+
您可以在 Grid Manager 中的 *节点* 页面上查看有关源设备节点当前 RAID 模式的信息。为设备选择 *存储* 选项卡。

*** 无法将具有 16 个对象存储卷（rangedb）的源节点克隆到具有较少对象存储卷的目标存储设备，即使目标设备的容量大于源节点。
...
不支持从没有节点加密的 SG110 源节点设备到具有节点加密的 SG110 设备目标的节点克隆。
...
您可以将 SG120 源节点设备克隆到 SG1200 服务设备目标，以赋予 Admin Node 或 Gateway Node 更大的功能。

不支持从没有节点加密的 SG120 源节点设备到具有节点加密的 SG120 设备目标的节点克隆。
...
不支持从没有节点加密的 SG1100 源节点设备到具有节点加密的 SG1100 设备目标的节点克隆。
...
您可以将 SG1200 源节点设备克隆到 SG120 服务目标设备。

* 将 SG1200 克隆到 SG120 服务目标设备可让您重新部署 SG1200 以用于要求更高的应用程序。例如，如果 SG1200 源节点设备用作管理节点，而您希望将其用作专用负载平衡节点。
* 将 SG1200 源节点设备替换为 SG120 服务目标设备会将网络端口的最大速度从 100-GbE 降低到 25-GbE。


不支持从没有节点加密的 SG1200 源节点设备克隆到具有节点加密的 SG1200 目标设备。
...
* 使用 DDP 或 DDP-16 配置的 SG5760 源节点设备_无法_克隆到具有匹配 RAID 模式的 SG6160 或 SG6260 存储目标设备。
...
* 如果驱动器大小和 RAID 模式相同，则可以将没有任何扩展盘架的 SG6060 源节点设备克隆到没有任何扩展盘架的 SG6160 或 SG6260 存储目标设备。
* 具有任意数量扩展盘架的 SG6060 源节点设备_无法_克隆到 SG6160 或 SG6260 存储目标设备，即使扩展盘架的数量、驱动器大小和 RAID 模式相同。
...
不支持从没有节点加密的 SGF6112 源节点设备克隆到具有节点加密的 SGF6112 或 SGF6212 设备目标。



[[sg6160-and-sg6260-appliance-cloning-information-and-restrictions]]
=== SG6160 和 SG6260 设备克隆信息和限制
