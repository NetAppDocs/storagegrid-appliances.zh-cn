...
summary: '对于当前使用 SANtricity 操作系统 08.42.20.00 (11.42) 或更高版本的存储控制器，您可以使用网格管理器或维护模式应用升级。' 
---
= 使用 Grid Manager 升级 SG6200 存储控制器上的 SANtricity 操作系统
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
直到SANtricity OS 升级成功应用于已选择进行升级的所有适用节点后，该过程才算完成。在每个节点上加载SANtricity OS（顺序）可能需要 30 多分钟，重新启动每个StorageGRID存储设备则可能需要长达 90 分钟。网格中不使用SANtricity OS 的节点不会受到此过程的影响。
...
NOTE: 在开始此过程之前，请应用最新的StorageGRID修补程序。看 https://docs.netapp.com/us-en/storagegrid/maintain/storagegrid-hotfix-procedure.html["StorageGRID 热修补程序操作步骤"^]了解详情。
...
** 选择一个或多个 *Approve* 按钮将一个或多个节点添加到 SANtricity 操作系统升级队列。如果 Status 不是 Nominal，则 *Approve* 按钮被禁用。
+
...
. 对于处于“完成”阶段且需要不同SANtricity OS 升级文件的节点，重复此升级过程。
+

NOTE: 对于状态为需要注意的节点，使用维护模式执行升级。



.相关信息
* https://mysupport.netapp.com/matrix["NetApp 互操作性表工具"^]
* link:upgrading-santricity-os-on-storage-controllers-using-maintenance-mode-sg6260.html["使用维护模式在 SG6200 控制器上升级 SANtricity 操作系统"]
