...
summary: 为了增加存储容量，您可以向部署在 StorageGRID 系统中的 SG6260 添加一个或两个扩展盘架。 
---
= 将扩展架添加到已部署的 SG6260
:allow-uri-read: 
:icons: font
...
[role="lead"]
为了增加存储容量，您可以向已部署在 StorageGRID 系统中的 SG6260 添加一个或两个扩展盘架。
...
完成一个或两个扩展架的过程每个设备节点应花费一个小时或更少的时间。为了最大限度地减少停机时间，以下步骤指示您在将 SG6260 置于维护模式之前安装新的扩展架和驱动器。剩余步骤每个设备节点大约需要 20 到 30 分钟。

.开始之前
* 您必须具有配置密码短语。
* 必须运行 StorageGRID 12.0 或更高版本。
* 每个扩展架都有扩展架和两根 SAS 缆线。
* 您有link:locating-sg6200-in-data-center.html["已物理定位控制器"]在数据中心中添加扩展架的位置。


.步骤
. 按照 link:../installconfig/sg6260-installing-60-drive-shelves-into-cabinet-or-rack.html["将包含60个驱动器的磁盘架安装到机柜或机架中"] 的说明进行操作。
. 从 Grid Manager 中，link:../commonhardware/placing-appliance-into-maintenance-mode.html["将 SG6200-CN 控制器置于维护模式"]。
. 如图所示、将每个扩展架连接到E4000控制器架。
+
此图显示了两个扩展架。如果只有一个，请将 IOM A 连接到控制器 A ，并将 IOM B 连接到控制器 B

+
image::../media/expansion_shelves_connections_sg6260.png[SG6260 扩展架连接]
...
+
可能显示在主页上的消息示例，具体取决于现有或新扩展盘架的数量，如下所示：

+
** 页面顶部显示的横幅指示检测到的扩展架总数。
+
...
+
重新启动完成后，将显示 *Tasks* 选项卡，其中包含重新启动节点或将设备置于维护模式的选择。

. 验证设备存储节点和新扩展架的状态。
+
.. 在 Grid Manager 中，选择 *Nodes* 并验证设备存储节点是否带有绿色复选标记图标。
+
