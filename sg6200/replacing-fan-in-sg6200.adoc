...
summary: SGF6212 设备和 SG6200-CN 控制器具有七个冷却风扇。如果其中一个风扇发生故障，您必须尽快更换，以确保设备有适当的冷却。 
---
= 更换 SGF6212 或 SG6200-CN (SG6260) 中的风扇
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
SGF6212 设备和 SG6200-CN 控制器具有七个冷却风扇。如果其中一个风扇发生故障，您必须尽快更换，以确保设备有适当的冷却。
...
* 您有link:locating-sg6200-in-data-center.html["物理定位 SGF6212 设备或 SG6200-CN 控制器"]，您正在数据中心中更换风扇。
+
...
. 将风扇连接器稳固地按入电路板。
. link:reinstalling-sg6200-cover.html["将顶盖放回产品上"]，然后按下闩锁以将盖子固定到位。
. link:power-sg6200-off-on.html["打开产品电源"] 并监控设备 LED 和启动代码。
+
使用 BMC 界面监控启动状态。
