...
. link:power-sg120-and-sg1200-off-on.html#shut-down-the-sg120-or-sg1200-appliance["关闭设备"]。
...
.. link:reinstalling-sg120-and-sg1200-into-cabinet-or-rack.html["从机柜或机架中取出发生故障的设备"]。
.. 请注意可更换组件（两个电源、七个冷却风扇、三个 NIC 和两个 SSD）在故障设备中的位置。
+
两个驱动器位于机箱中的以下位置(所示为已卸下挡板的机箱正面)：

+
image::../media/sg120_sg1200_ssds_locations.png[驱动器位置]
...
CAUTION: 如果您希望保留驱动器上的数据，请将 SSD 驱动器插入到故障设备中占用的相同驱动器插槽中。否则，Appliance Installer 将显示一条警告消息，您必须将驱动器放入正确的插槽中并重新启动设备，然后设备才能重新加入网格。

.. link:reinstalling-sg120-and-sg1200-into-cabinet-or-rack.html["将替代设备安装到机柜或机架中"]。
.. 更换缆线和任何光纤收发器。


. 打开产品电源。
. 如果您更换的设备已为SED驱动器启用硬件驱动器加密、则必须执行此操作 link:../installconfig/optional-enabling-node-encryption.html#access-an-encrypted-drive["输入驱动器加密密码短语"] 在更换设备首次启动时访问加密驱动器。
...
* link:../installconfig/troubleshooting-hardware-installation-sg120-and-sg1200.html#view-error-codes["查看设备的启动代码"]
