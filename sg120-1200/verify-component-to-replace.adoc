...
.关于此任务
使用此过程可识别硬件出现故障的设备，以及哪些可更换硬件组件无法正常运行。可能被确定需要更换的组件包括：
...
+
.. 在网格管理器中，选择 *节点* 。
.. 从节点页面的表格中，选择具有故障组件的 StorageGRID 设备名称。
.. 选择 * 硬件 * 选项卡。
+
检查StorageGRID 设备部分中的*计算控制器序列号*。检查此序列号是否与要更换组件的存储设备的序列号匹配。如果序列号匹配、则表示您找到了正确的设备。

+
*** 如果 Grid Manager 中的 StorageGRID Appliance 部分未显示，则所选节点不是 StorageGRID 设备。从树视图中选择其他节点。
...
.相关信息
link:turning-sg120-and-sg1200-identify-led-on-and-off.html["打开设备识别LED"]
