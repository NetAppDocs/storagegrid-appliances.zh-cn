...
---
= 为 SG6260、SG6160、SG6000、SG5700 和 SG5800 StorageGRID 设备配置 SANtricity
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
以下 StorageGRID 设备可以访问 SANtricity System Manager：
...
NOTE: 如果您的设备启用了驱动器安全功能，则必须设置并有权访问 SANtricity System Manager。
...
** 使用网格管理器并选择 *节点* > `*_appliance Storage Node_*` > *SANtricity System Manager*
...
.SG6060
--
对于 SG6060 存储设备，只有在安装存储设备之前配置了密钥管理，才能在 SSD 驱动器上自动启用驱动器安全性。
...
.. 对于 E2800 控制器架，创建一个安全密钥（控制器和驱动器共享的用于读写访问的一串字符串）。
+
...
.SG6260
--
SG6260 存储设备可以在 SG6200-CN 计算控制器和 E4000 控制器架中配备符合 FIPS 的驱动器。驱动器加密分别为 SG6200-CN 驱动器和 E4000 驱动器配置。

.. link:optional-enabling-node-encryption.html#drive-encryption["启用驱动器加密"] 适用于安装在 SG6200-CN 计算节点中的 SED SSD。
.. 创建安全密钥(控制器和驱动器共享的字符串、用于进行读/写访问)。
+
*** 您可以使用控制器的永久性内存、也可以 https://docs.netapp.com/us-en/e-series-santricity/sm-settings/create-internal-security-key.html["创建内部密钥"^]使用密钥管理服务器提供的外部密钥。
*** 要使用密钥管理服务器提供的外部密钥、必须首先 https://docs.netapp.com/us-en/e-series-santricity/sm-settings/overview-drive-security.html["使用密钥管理服务器建立身份验证"^]在SANtricity系统管理器中使用。
...
NOTE: 仅当您无法从 StorageGRID Appliance Installer（*Advanced* > *SANtricity System Manager*）或 Grid Manager（*Nodes* > *SANtricity System Manager*）访问 SANtricity System Manager 时，才执行此过程。
