...
. 继续监控安装进度，直到*安装StorageGRID*阶段暂停并且嵌入式控制台上出现一条消息，提示您使用网格管理器在管理节点上批准此节点；然后，转到下一步。
. https://docs.netapp.com/us-en/storagegrid/admin/signing-in-to-grid-manager.html["转到网格管理器"^]主管理节点，批准待处理的存储节点，并完成StorageGRID安装过程。
+
...
. 继续监控安装进度，直到出现以下过程之一：
+
** 对于除主管理节点之外的所有设备节点，“安装StorageGRID”阶段将暂停，并且嵌入式控制台上会出现一条消息，提示您使用 https://docs.netapp.com/us-en/storagegrid/admin/signing-in-to-grid-manager.html["网格管理器"^]；然后，转到下一步。
