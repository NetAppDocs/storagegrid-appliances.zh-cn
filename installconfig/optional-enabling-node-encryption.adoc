...
此外，在启用节点加密后启用 FIPS 模式时， NetApp StorageGRID Kernel Crypto API 6.1.129-1-ntap1-amd64 模块用于静态数据加密。参考 https://review.docs.netapp.com/us-en/storagegrid_storagegrid-120-dev/admin/manage-tls-ssh-policy.html#select-a-security-policy["选择安全策略"^]了解更多信息。
...
CAUTION: 当安装启用节点加密的设备时，会分配一个临时密钥。在设备连接到密钥管理系统 (KMS) 并设置 KMS 安全密钥之前，设备上的数据不受保护。请参阅 https://docs.netapp.com/us-en/storagegrid/admin/kms-overview-of-kms-and-appliance-configuration.html["Kms设备配置概述"^]了解更多信息。
...
驱动器加密可用于安装在 SG100、SG1000、SG110、SG1100、SG120、SG1200、SGF6112、SGF6212、SG6100-CN 或 SG6200-CN 计算节点或控制器中的任何 SED SSD。

* 对于服务设备、SSD是节点根磁盘。
* 在 SG6100-CN 或 SG6200-CN 控制器中，SSD 用于缓存。
* 在 SGF6112 或 SGF6212 中，SSD 是节点根磁盘，用于对象数据的主存储。
...
当您为具有 FIPS 驱动器的StorageGRID设备启用驱动器加密时，FIPS 驱动器提供的 FIPS 加密将用于静态数据的加密。

您可以在加载 Grid Manager 之前在初始设备安装期间启用驱动器加密。您还可以通过将设备置于维护模式来启用驱动器加密或更改密码。
